...
% 
 clear; close all;
 run('E:\database\AED-MARVL-v0.4\Projects\Coorong\MARVL.m');
...
%                 rawdata.data.TPTN = TN./TP;
%                 rawdata.data.TPTN(TN==0)=16;
%                 clear TN TP;
%                 
            else
                rawdata=load_AED_vars(master.ncfile,mod,loadname,allvars);
    ...
    for site = sites
        ...
        for mod = 1:length(master.ncfile)
            if (isfield(t_data(mod).fields,loadname) || find(contains({'TNTP','PONDON','DINDIP'},loadname))>0)
                ...
                if strcmp(loadname,'TNTP')
                    isConv=1;
                    c_units='-';
                elseif strcmp(loadname,'PONDON')
                    isConv=1;
                    c_units='-';
                end
