%function marvl_plot_timeseries(MARVLs)
%**************************************************************************
%
...
MARVLs=yaml.loadFile('E:\database\AED-MARVl-v0.2\Examples\Cockburn\MARVL.yaml',...
    "ConvertToArray", true);
master=MARVLs.master;
config=MARVLs.timeseries;

MARVLs_auto=yaml.loadFile('E:\database\AED-MARVl-v0.2\Examples\Cockburn\MARVL.yaml');
...
if config.plotmodel==1
    [t_data, d_data]=pre_load_model_GEOs(master.ncfile);
end
...
for var = config.start_plot_ID:config.end_plot_ID
    ...
    for site = sites
        ...
        for mod = 1:length(master.ncfile)
            if isfield(t_data(mod).fields,loadname) && config.plotmodel
                tStart = tic;
                ...
                [~,~,~,~,MatchedData0] = ...
                    ...
                    isvalidation,mod,shp,site,loadname,var,master,layer);
                if ll==1
        ...
        %% adding error output
        %disp(isvalidation);disp(MatchedData)
                    ...
                    if config.plotvalidation
                        ...
                        if fgf > 1
                            fp = plot(xdata_d,ydata_d,mface,'markeredgecolor',...
                                config.ncfile(mod).edge_color{2},'markerfacecolor',...
                            ...
                            fp = plot(xdata_d,ydata_d,mface,'markeredgecolor',...
                                config.ncfile(mod).edge_color{2},'markerfacecolor',mcolor,...
                        ...
                        if fgf > 1
                            fp = plot(xdata_d,ydata_d,mface,'markeredgecolor',...
                                config.ncfile(mod).edge_color{1},'markerfacecolor',...
                                mcolor,'markersize',3,'HandleVisibility','off');hold on
                            uistack(fp,'top');
                            if config.validation_minmax
                                fp = plot(xdata_d,ydata_max_d,'+','color',[0.6 0.6 0.6],...
                            ...
                            fp = plot(xdata_d,ydata_d,mface,'markeredgecolor',...
                                config.ncfile(mod).edge_color{1},'markerfacecolor',mcolor,...
...
if config.plotmodel
    if strcmpi(layer,'bottom') == 1
        plot(xdata,ydata,'color',config.ncfile(mod).colour{2},'linewidth',0.5,...
        'DisplayName',[master.ncfile(mod).legend,' (Bot Median)'],...
        'linestyle',config.ncfile(mod).symbol{2});hold on;
    else
        
    plot(xdata,ydata,'color',config.ncfile(mod).colour{1},'linewidth',0.5,...
        'DisplayName',[master.ncfile(mod).legend,' (Surf Median)'],...
...
if config.plotmodel && config.isModelRange == 1
    ...
    set(fig,'DisplayName',[master.ncfile(mod).legend,' (Bot Range)']); %Surf
    ...
    set(fig,'DisplayName',[master.ncfile(mod).legend,' (Surf Range)']); %Surf
    set(fig,'FaceAlpha', config.alph);
    hold on;
