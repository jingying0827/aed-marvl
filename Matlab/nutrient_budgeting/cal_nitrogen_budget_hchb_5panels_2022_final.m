...
% time and output folder
    ...
    outputfolder='./Budget_EB_Nitrogen/';
	...
	% nitrogen in primary production
    PP_vars={'WQ_DIAG_MAG_IN_BEN',...
        'MPB_BENXNC',...
        'WQ_DIAG_PHY_IN'};
    
    for p=1:length(PP_vars)
        if p==1
            data.PP_NIT=data.(PP_vars{p});
        else
            data.PP_NIT=data.PP_NIT+data.(PP_vars{p});
        end
    end
    
	% calculate total sediment diffusive exchange, denitrification, and burial
    data.NIT_release=data.WQ_DIAG_NIT_AMM_DSF+data.WQ_DIAG_OGM_DON_SWI+data.WQ_DIAG_NIT_NIT_DSF;
    data.NIT_denit=data.WQ_DIAG_NIT_DENIT;
    ...
    datesv=datenum(datess,'yyyymmdd');
    set(gca,'XTick',datesv,'XTickLabel',datestr(datesv,'mm-yy'));
    title('(b) nitrogen fluxes at sediment-water interface (+ve: into water; -ve: into sediment)','FontWeight','Bold','FontSize',fs);
    ...
    data.NIT_uptake=data.WQ_DIAG_PHY_UPT_NO3+data.WQ_DIAG_PHY_UPT_NH4;
   % for dd=1:length(data.NIT_uptake)
   %     if data.NIT_uptake(dd)==0
   %         data.NIT_uptake(dd)=(data.NIT_uptake(dd-1)+data.NIT_uptake(dd+1))/2;
   %     end
   % end
    ...
    datesv=datenum(datess,'yyyymmdd');
    set(gca,'XTick',datesv,'XTickLabel',datestr(datesv,'mm-yy'));
    title('(c) nitrogen uptake, atmospheric depositoin, and denitrification fluxes','FontWeight','Bold','FontSize',fs);
    xlabel('');ylabel({'nitrogen fluxes','(tonnes/day)'});
	...
	data.nsnetflux.IN=fluxdata.data.Bound1.IN + fluxdata.data.Bound2.IN + ...
            fluxdata.data.Bound3.IN + fluxdata.data.Gra.IN;
    data.nsnetflux.ON=fluxdata.data.Bound1.ON + fluxdata.data.Bound2.ON + ...
            fluxdata.data.Bound3.ON + fluxdata.data.Gra.ON;
    data.nsnetflux.PPN=fluxdata.data.Bound1.PPN + fluxdata.data.Bound2.PPN + ...
            fluxdata.data.Bound3.PPN + fluxdata.data.Gra.PPN;
 ...
 %   end
    ...
    DeltaSed=DeltaWat+data.NIT_denit-Ttran-data.NIT_release; %
    ...
    hold on;
    plot(t1:t2,-(Sedacc+Releaseacc)*14/1e9,'Color',[136,86,167]./255);
    hold on;

    text(t2+5,Watacc(end)*14/1e9,num2str(Watacc(end)*14/1e9,'%4.0f'), 'Color',[67,162,202]./255);
    hold on;
    text(t2+5,-double((Sedacc(end)+Releaseacc(end)))*14/1e9,num2str(-(Sedacc(end)+Releaseacc(end))*14/1e9,'%4.0f'), 'Color',[136,86,167]./255);
    hold on;
