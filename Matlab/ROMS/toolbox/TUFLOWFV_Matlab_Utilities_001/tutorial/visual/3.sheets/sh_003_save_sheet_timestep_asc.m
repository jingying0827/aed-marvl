% sh_003_save_sheet_timestep_asc.m
%
% Plot an interactive sheet plot and save the output as an asci grid at a
% particular timestep.
%
% SDE 2018
...
bounds      = [ 159.0166  -31.4418;
				159.1859  -31.3575];
grd         = 20;                                                           % resolution (meters)
...
%-------------------- save figure ------------------
outname = sprintf('%s_%s.asc',sheet_var,datestr(time_out,'yyyymmddHHMMSS'));% name of output
set(fvobj,'TimeCurrent',time_out);                                          % Set the model time 
set(fvobj.SliderObj,'Visible','off');                                       % hide the slider bar

fv2ascii_v2(tfv_resfile,h.ResObj.ResultsCell.(sheet_var),grd,'bounds',bounds,'outfil',[out_plots outname]); % save the asci grid
