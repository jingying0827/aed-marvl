function [fielddata,fielddist] = marvl_getfielddata_boxregion_dist(fdata,shp,config,def,varname,tim)



isSurf=config.isSurf;
isSpherical=config.isSpherical;
mtime=def.pdates(tim).value;

%(fdata,shp,def,isSurf,varname,mtime,isSpherical)
...
for i = 1:length(shp)
    if isSpherical
    [sdata(i,1),sdata(i,2)]=ll2utm(shp(i).X,shp(i).Y);
    else
    sdata(i,1) = shp(i).X;
    sdata(i,2) = shp(i).Y;
	end
